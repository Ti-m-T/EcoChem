...
class Chemical:  # Class grouping all chemicals
    ...
    def get_pictograms(self) -> list:
        ...
        def search_pictograms(
            section,
        ):  # Recursive function to search for GHS pictograms.
            ...
            if isinstance(section, dict):  # We ask if the section is a dictionary.
                ...
                for (
                    ...
                    section.values()
                ):  # If it is a dictionary we loop through its values to find the GHS pictogram codes.
                    ...
                    if isinstance(values, str):  # We check if the values are strings.
                        ...
                        for (
                            ...
                            mapping.items()
                        ):  # We loop through the mapping of GHS pictogram codes and names.

                            if (
                                code in values
                            ):  # If we find a GHS pictogram code in the value string.

                                self.pictograms.add(
                                    name
                                )  # We add the name of the pictogram to the list of pictograms.

                    else:
                        search_pictograms(
                            values
                        )  # If the values are not strings we do a recursive call on them to keep searching for GHS pictogram codes in the database.
                ...
                for item in section:  # If it is a list we loop through its items.
                    ...
                    search_pictograms(
                        item
                    )  # We do a recursive call on the items to keep searching for GHS pictogram codes in the database.

        search_pictograms(
            data
        )  # We execute the search_pictograms function on the data we got from the PubChem database of the molecule wanted.
